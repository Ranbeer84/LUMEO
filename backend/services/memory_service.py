...
FIXES APPLIED:
- Added `import time` (was missing; get_conversation_timeline used time.time()
  which caused NameError at runtime)
"""

from typing import List, Dict, Optional, Set
import logging
import time                          # FIX: was missing — caused NameError in get_conversation_timeline
...
class MemoryService:
    ...
    def extract_conversation_topics(self, session, conversation_id: str) -> List[str]:
        ...
        for msg in messages:
            ...
            if msg.retrieved_photo_ids:
                # FIX: column is meta_data, not metadata
                meta = msg.meta_data or {}
                if 'filters' in meta:
                    for key, value in meta['filters'].items():
    ...
    def build_memory_context(self, relevant_memories: List[Dict]) -> str:
    ...
    def get_conversation_timeline(
        ...
        # FIX: `time` was not imported before, so time.time() raised NameError
